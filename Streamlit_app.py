...
# ---------------------------- CONFIGURATION ----------------------------
EXCHANGE_SYMBOL = 'BTC/USDT'         # Binance pair used for both data and trading
MAX_RISK_PERCENT = 1.0
DEFAULT_BALANCE = 1000

# ---------------------------- FETCH LIVE DATA FROM BINANCE (public, no keys) ----------------------------
...
def load_data(symbol='BTC/USDT', limit=120):
    ...
    placeholder.info("Fetching latest market data from Binance… (this takes a few seconds)")
    try:
        # Since 1h candles: we need roughly 5 days = 120 hours
        ...
        placeholder.empty()
        if df.empty:
            st.error("Binance returned no data. Please try again later.")
            return pd.DataFrame()
        return df
...
def select_best_strategy(df):
    recent = df.iloc[-48:]   # last 48 hours
    best_score = -np.inf
    best_name = None
    best_pf = None
...
def get_exchange(mode):
    if mode == 'Paper':
        ...
        if 'DEMO_API_KEY' not in st.secrets or 'DEMO_SECRET_KEY' not in st.secrets:
            st.error("Demo API keys missing in Streamlit Secrets. Add DEMO_API_KEY and DEMO_SECRET_KEY.")
            return None
        ...
        if 'LIVE_API_KEY' not in st.secrets or 'LIVE_SECRET_KEY' not in st.secrets:
            st.error("Live API keys missing in Streamlit Secrets.")
            return None
        return ccxt.binance({
...
mode = st.sidebar.selectbox(
    "Trading Mode",
    options=["Paper", "Demo", "Live"],
    index=0,
    help="📄 Paper = simulation, 🧪 Demo = Binance Testnet, 🚀 Live = real money"
)

if mode == "Demo":
    st.sidebar.info("🧪 Demo Mode: uses Binance Testnet with fake money.")
elif mode == "Live":
    st.sidebar.warning("🚀 Live Mode: real orders on Binance. Use with caution.")
else:
    st.sidebar.info("📄 Paper Mode: no orders sent, just simulation.")
...
# Main area – data loading from Binance (no yfinance)
...
if best_strat is None:
    st.warning("No suitable strategy found for current market. Try again later.")
else:
        ...
        if st.session_state.get("confirm"):
            ...
            if mode == "Paper":
                ...
                if st.button("🚀 Execute Trade"):
                    ...
                    if success:
                        st.success(f"{mode} trade placed! Check your Binance account.")
                        st.balloons()
