...
def load_data():
    try:
        ...
        # Convert all numeric columns to Python float (makes everything plain float)
...
if best_strat is None:
    ...
    if direction == "NEUTRAL":
        ...
        latest = df.iloc[-1]
        # Extract and IMMEDIATELY convert to plain Python float
        ...
        # These are already Python floats because entry and atr are
        risk_amount = float(balance * (risk_pct / 100))
        distance = abs(entry - stop)
        size = float(risk_amount / distance) if distance > 0 else 0.0
        size = round(size, 5)

        # DEBUG: if error still occurs, uncomment the next line to see types
        # st.write(f"Types: entry={type(entry)}, stop={type(stop)}, target={type(target)}, risk={type(risk_amount)}, size={type(size)}")
