...
# ---------------------------- CONFIGURATION ----------------------------
EXCHANGE_SYMBOL = 'BTC/USDT'         # Binance pair for data & trading
...
def load_data(symbol='BTC/USDT', limit=120):
    """Fetch hourly OHLCV from Binance public API (retry once)."""
    exchange = ccxt.binance({'enableRateLimit': True})
    msg = st.empty()
    msg.info("Fetching latest market data from Binance…")
    for attempt in range(2):
        try:
            ...
            if attempt == 0:
                ...
                msg.empty()
                st.error(f"Could not fetch data from Binance: {e}")
                return pd.DataFrame()
...
if best_strat is None:
    ...
    if direction == "NEUTRAL":
        ...
        # ---- FIX: convert all numeric values to plain Python float ----
        entry = float(entry)
        stop = float(stop)
        target = float(target)
        atr = float(atr)   # not displayed, but safe
