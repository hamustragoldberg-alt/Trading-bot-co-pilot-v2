...
# ---------------------------- DATA LOADING (LOCAL CSV – NO NETWORK) ----------------------------
@st.cache_data(ttl=60, show_spinner=False)
def load_data():
    try:
        df = pd.read_csv('btc_data.csv')
        # Yahoo export has columns: Date,Open,High,Low,Close,Adj Close,Volume
        ...
        st.error(f"Could not load local data file: {e}")
        return pd.DataFrame()
...
df = load_data()
if df.empty:
    st.error("Data file missing or corrupt. Make sure 'btc_data.csv' is in the repository.")
    st.stop()
